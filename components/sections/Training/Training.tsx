...
export const Training: React.FC<TrainingProps> = ({ data }) => {
  ...
  return (
    <Flex direction="column" className="py-9 md:py-14">
      ...
      <Flex className="flex-wrap flex-col lg:flex-row gap-4 justify-between w-full">
        {programs.map((program) => {
          return (
            <Flex
              key={program.title}
              justify="between"
              className="flex-col md:flex-row rounded-design border-[3px] border-primary md:h-[285px] w-full lg:max-w-[calc(50%-8px)] overflow-hidden"
            >
              <Flex
                direction="column"
                justify="between"
